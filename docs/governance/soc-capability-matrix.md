...
| id | area | capability | sub-capability | priority | owner_role | status | evidence_paths | acceptance_criteria (DoD) | next_action |
|---|---|---|---|---|---|---|---|---|---|
| OLA1-LNX-001 | Wave 1 | Linux endpoint | Provision VM + hardening baseline | P0 | SOC Engineer | DONE | docs/infra/ola1-linux-endpoint.md; artifacts/infra/ola1-linux-endpoint_YYYY-MM-DD.md | Linux VM created, SSH keys, updates, minimal hardening, time sync; documented | Lab mode accepted (password SSH + no host firewall); tighten before production-like exposure |
