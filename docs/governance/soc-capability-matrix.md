...
| MET-002 | Metrics | Trend tracking | Baselines & drift | P1 | Telemetry Owner | DONE | tools/metrics/windows_channel_baseline_drift.sh; artifacts/telemetry/baselines/*; docs/governance/metrics.md | Baselines for key channels, and alert on deviation | Start with Windows, extend to Linux/pfSense |
...
| OLA1-LNX-002 | Wave 1 | Linux telemetry | Wazuh agent installed + enroll | P0 | SOC Engineer | DONE | docs/runbooks/onboarding/linux/onboarding_linux.md; artifacts/onboarding/linux/* | Agent enrolled and visible; events arriving | Continue with OLA1-LNX-003 Linux DQ gate |
| OLA1-LNX-003 | Wave 1 | Linux DQ | Linux onboarding gate (pass/fail) | P0 | Telemetry Owner + SOC Engineer | DONE | tools/onboarding/linux/linux_onboarding_audit.sh; tools/onboarding/linux/create_issue_on_fail.sh; artifacts/onboarding/linux/* | Gate validates auth.log/syslog (or journald), agent active, syscollector, SCA; exit code + Issue on fail | Promote M6 (SCA) from WARN to MUST after 1-2 stable cycles |
...
| OLA1-OPS-002 | Wave 1 | Metrics | Implement MET-001 weekly report | P0 | Telemetry Owner | DONE | docs/governance/metrics.md; tools/metrics/weekly_metrics.sh | Weekly report produced and stored in artifacts | Keep weekly generation discipline and connect with MET-002 drift tracking |
